...
public class CardCountService {
    ...
    public int getMintCount(String url) {
        ...
        while (moreDataAvailable) {
            //Result nextBatch = new Result();
            logger.info("RESULT BEFORE NEXT RETRIEVE: {}", result.getResult().length);
            try {
                result = retrieveDetails(urlWithCursor);
                logger.info("RESULT AFTER NEXT RETRIEVE: {}", result.getResult().length);
            } catch (Exception e) {
                logger.error("failed to retrieve next batch");
            }


            logger.info("nextBatchCount: {}", result.getResult().length);
            runningTotal += result.getResult().length;
